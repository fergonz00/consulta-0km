...
- Rechazada → precio_max_admin. ⚠️ El estado NO se manda como "Rechazada" (15-09-2026, Fer: *"no tiene sentido que diga el monto autorizado si salió rechazada"*): en el 0km "No acepto" es **solo no acepto**: ya no pide precio, guarda `precio_max_admin = null` y el WhatsApp dice "No se aceptó el precio pedido" con monto "—". Si hay un precio más alto para ofrecer, es **Contraoferta**. En `venta_hecha` rechazada va "No se autoriza la transferencia (tiene que pagar por SICE) o debe pagar el 1,5% correspondiente en función de la transferencia": el % sale de `costos_financieros_mes` (deb/cred + SIRCREB, mes en curso o el último anterior, misma regla que el front). Usados sigue pidiendo mejor precio en "No acepto".
